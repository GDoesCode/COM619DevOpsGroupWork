...
async function onStart(){
    try{
    const response = await fetch (`https://172.167.64.210:8080/login`)
...
async function signUp(newUser) {
    try {
        const response = await fetch('https://172.167.64.210:8080/signup', {
            method: 'POST',
            headers: {
...
async function ajaxSearch(regionIn){
    try{
        if (regionIn != null){
            if(regionIn != ""){
        const response = await fetch(`https://172.167.64.210:8080/poi/region/${regionIn}`);
...
async function addPOI(poiIn){
    try{
        const response = await fetch(`https://172.167.64.210:8080/poi/create`,{
            method: 'POST',
            headers: {
                'Content-Type' : 'application/json'
            },
...
async function recPoi(poi){
    try{
        const response = await fetch(`https://172.167.64.210:8080/poi/recommend/${poi.id}`,{
            method:"POST"
        })
...
async function revPoi(poi_id){
    ...
    try{ 
            ...
            const response1 = await fetch(`https://172.167.64.210:8080/poi/review`,{
                method:"POST",
                headers:{
                    'Content-Type' : 'application/json'
                },
...
async function uploadPhoto(POIID){
    ...
    if(photoFiles == 0){
        ...
        const response = await fetch(`https://172.167.64.210:8080/photo/upload/${POIID}`,{
            method:"POST",
            body: formData
        })
        if(response.status == 200){
            ...
            img.src = `https://172.167.64.210:8080/uploadPics/${POIID+photoFiles[0].name}`
            src.appendChild(img)
...
async function logout(){
    try{
    const response = await fetch(`https://172.167.64.210:8080/logout`, {method:"POST"})
